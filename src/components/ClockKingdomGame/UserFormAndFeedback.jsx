...
const UserFormAndFeedback = ({
  ...
  const generatePDF = () => {
    ...
    doc.text("Clock Kingdom Feedback Report", 20, 20);
    ...
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(`Final Score: ${score}`, 20, 110);

    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Incorrect Answers", 20, 130);
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");

    if (wrongAnswers.length === 0) {
      doc.text("No incorrect answers! Great job!", 20, 140);
    } else {
      wrongAnswers.forEach((item, index) => {
        doc.text(`${index + 1}. Game: ${item.game}`, 20, 140 + index * 15);
        doc.text(`   Question: ${item.question}`, 20, 145 + index * 15);
        doc.text(
    ...
    doc.save("ClockKingdom_Feedback.pdf");
  };

  if (gameState === "welcome" && !formSubmitted) {
    return (
      ...
      <div className="feedback-container">
        <div className="feedback-content">
          ...
          <div className="game-results">
            ...
            <p>
              Badges Earned: {badges.length} ({badges.join(", ")})
            </p>
          </div>
